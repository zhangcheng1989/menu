...
@interface ZCMenu(){
    ...
    NSMutableArray *menuNameLabelArray;
...
- (void)menuButtonClicked:(UIButton *)btn{
    if ([self.delegate respondsToSelector:@selector(menuSelected:atIndex:)]) {
        NSInteger buttonTag =  btn.tag;
        for (int index = 0; index < menuButtonArray.count; index++) {
            ...
            if (menuButton.tag == buttonTag) {
                ...
                if (self.delegate) {
                    [self.delegate menuSelected:menuName atIndex:index];
                }
                break;
            }
        }
